...
namespace blazorserver01.Data
{
    public class Environment
    {
        ...
        public int surroundingNeighborns(int i, int j, String specie){
            int ans = 0;
        ...
        public void next_Rabbit_Carrot_Step(){
            ...
            for(var i=0; i<this.rows; i++)
            for(var j=0; j<this.cols; j++){
                aux[i,j] = null;//rule2, 5
                //rule 1
                if(this.specie(this.cell[i,j])=="Carrot"){
                    if(this.surroundingNeighborns(i,j,"Rabbit")==0){
                ...
                //rule 4
                    ...
                    if(this.cell[i,j]==null){
                        if(this.surroundingNeighborns(i,j,"Rabbit")>=2)
                            aux[i,j] = new Rabbit(i,j,this);
                        else if(this.surroundingNeighborns(i,j,"Carrot")>=3)
